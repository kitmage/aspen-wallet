        ...
        foreach ($conflictItems as $item) {
            ...
            foreach ($remoteCalendarIds as $calendarId) {
                ...
                foreach ($bookedEvents as $event) {
                    ...
                    $travelDuration = AppleHelper::getTravelDuration($event) * 60;
                    $eventStart = gmdate('Y-m-d H:i:s', strtotime($event->dtstart) - $travelDuration);
                    $eventEnd = gmdate('Y-m-d H:i:s', strtotime($event->dtend));

                    if ($isUtc = strpos($event->dtstart, 'Z')) {
                        $eventStart = DateTimeHelper::convertFromUtc($eventStart, $toTimeZone);
                        $eventEnd = DateTimeHelper::convertFromUtc($eventEnd, $toTimeZone);
                    }
