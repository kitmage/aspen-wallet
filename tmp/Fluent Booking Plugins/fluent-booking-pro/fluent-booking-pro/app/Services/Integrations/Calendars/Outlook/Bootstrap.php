...
class Bootstrap extends BaseCalendar
{
    ...
    public function getBookedSlots($books, $calendarSlot, $toTimeZone, $dateRange, $hostId, $isDoingBooking)
    {
        ...
        foreach ($items as $item) {
            ...
            foreach ($item['check_ids'] as $remoteId => $calDetails) {
                $cacheKey = md5($cacheKeyPrefix . '_' . $remoteId);
                $remoteSlots = CalendarCache::getCache($meta->id, $cacheKey, function () use ($calendarApi, $startDate, $endDate, $remoteId, $calDetails) {
                    $events = $calendarApi->getCalendarEvents($remoteId, [
                        'startDateTime' => $startDate,
                        'endDateTime'   => $endDate,
                        'isHoliday'     => Arr::get($calDetails, 'is_holiday', false)
                    ]);
    ...
    public function createEvent($config, Booking $booking)
    {
        ...
        if ($booking->getMeta('__outlook_calendar_event') && (!$booking->isMultiGuestBooking() && !$booking->isRoundRobinBooking())) {
