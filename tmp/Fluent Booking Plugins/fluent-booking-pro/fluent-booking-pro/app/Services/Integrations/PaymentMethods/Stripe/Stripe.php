...
class Stripe extends BasePaymentMethod
{
    ...
    public function makePayment($orderItem, $calendarSlot, $order)
    {
        $hash = $orderItem->hash;
        ...
        $paymentTotal = $this->getPayableAmount($items, $isZeroDecimal) * $quantity;

        $discountTotal = $isZeroDecimal ? round($order->discount_total / 100, 2) : $order->discount_total;

        $paymentArgs = [
            'client_reference_id' => $hash,
            'items'               => $items,
            'amount'              => (int)round($paymentTotal - $discountTotal),
    ...
    public function confirmStripePayment()
    {
        ...
        $currency = Arr::get($response, 'currency', $order->currency);
        $isZeroDecimal = CurrenciesHelper::isZeroDecimal($currency);

        $orderAmount = intval($order->total_amount);
        $expectedAmount = $isZeroDecimal ? intval($orderAmount / 100) : $orderAmount;
        ...
        $updateData = [
            ...
            'total_paid'       => $isZeroDecimal ? ($amount * 100) : $amount,
    ...
    public function refundPayment($orderItem, $calendarSlot)
    {
        ...
        $currency = $order->currency ?? CurrenciesHelper::getGlobalCurrency();
        $isZeroDecimal = CurrenciesHelper::isZeroDecimal($currency);

        $refundAmount = intval($transaction->total);
        $stripeRefundAmount = $isZeroDecimal ? intval($refundAmount / 100) : $refundAmount;
    ...
    public function getPayableAmount($items, $isZeroDecimal = false)
    {
        $total = 0;
        ...
        return $isZeroDecimal ? $total : $total * 100;
    }
