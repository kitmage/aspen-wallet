...
$app->addFilter('fluent_booking/save_event_booking_field_payment', function ($field, $value, $calendarEvent) {
    return PaymentHelper::updatePaymentFieldSaveData($field, $value, $calendarEvent);
}, 10, 3);
...
$app->addFilter('fluent_booking/settings_menu_items', function ($items) {
    ...
    $items['payment_settings']['disable'] = false;

    foreach (['payment_settings', 'payment_methods', 'payment_coupons'] as $child) {
        if (!empty($items['payment_settings']['children'][$child])) {
            $items['payment_settings']['children'][$child]['disable'] = false;
        }
    }
